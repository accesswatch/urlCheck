...
class acrBuilder:
    ...
    def writeDocx(cls, pathParent, lFolders, lPageBuckets, lPageNames, lOrderedCrits, dtRunStart=None):
        ...
        per-rule focus. Sections:
          ...
          2. Overview, including:
             2a. Applicable standards (WCAG 2.2 + Revised Section 508)
             2b. Evaluation methods used (automated + manual)
             2c. Terms (definitions of conformance levels)
          ...
          6. Methodology (Calc precedence, accessibility failure rate)
          7. Resources (WCAG, axe, WebAIM, VPAT, Section 508 links)
          8. About this report (project home, scope disclaimer)

        When no pages were scanned in this run, sections 2-6 are
        replaced by a single "No pages were scanned in this run"
        notice that explains the situation honestly, and section 7
        (Resources) and 8 (About) are skipped. This avoids the
        misleading "all 86 criteria are Not Evaluated" appearance
        that would otherwise result from running the full report
        machinery against an empty page set.
        ...
        # If the run produced no scannable pages, the rest of this
        # document would be misleading: a "Conformance Summary" with
        # all 86 criteria marked Not Evaluated implies a real test
        # was performed and yielded no information, when in fact no
        # test was performed. So we stop here, write a clear notice,
        # and skip the conformance/attention/methodology sections.
        if not lFolders:
            d.add_heading("No pages were scanned in this run", level=1)
            d.add_paragraph(
                f"This report was generated by {sProgramName} but contains "
                f"no per-page accessibility data. Either no pages were "
                f"successfully scanned during this session, or the per-page "
                f"results files (one per scanned page, named "
                f"\"{sJsonName}\") could not be located beneath the parent "
                f"output folder."
            )
            d.add_paragraph(
                f"To produce a complete report, scan at least one page "
                f"with {sProgramName}, confirm that each page's output "
                f"folder contains a {sJsonName} file, then re-run "
                f"{sProgramName} (or the ACR generator) so this document "
                f"can be regenerated with real conformance data."
            )
            d.add_paragraph(
                f"For reference: ACR.xlsx alongside this document still "
                f"contains the full template for all 86 WCAG 2.2 success "
                f"criteria with manual-test guidance, ready to fill in "
                f"once pages have been scanned."
            )
            ...
            d.save(str(pathDocx))
            logger.info(f"ACR: wrote {pathDocx} (no-pages template)")
            return
        ...
        d.add_paragraph(
            f"This report is a draft generated by automation. It is "
        ...
        # ---- Section 2a: Applicable standards ----
        # Modeled after the "Applicable Standards/Guidelines" section in
        # mainstream VPATs (e.g. Zoom v2.3): a small, explicit list of
        # what this report is scoped to, so a reviewer doesn't have to
        # infer it from context.
        d.add_heading("Applicable standards", level=2)
        d.add_paragraph(
            "This report covers conformance with the following standards:"
        )
        for sStd in [
            "Web Content Accessibility Guidelines (WCAG) 2.2, Levels A and AA, including all 86 success criteria",
            "Revised Section 508 standards (29 U.S.C. 794d), to the extent that Section 508 incorporates WCAG by reference",
        ]:
            p = d.add_paragraph(style="List Bullet")
            p.add_run(sStd)
        d.add_paragraph(
            "Where a WCAG criterion is satisfied (Supports), the corresponding "
            "Section 508 requirement that incorporates that criterion is also "
            "considered satisfied."
        )

        # ---- Section 2b: Evaluation methods used ----
        d.add_heading("Evaluation methods used", level=2)
        d.add_paragraph(
            f"Automated testing: each target page was loaded in "
            f"Microsoft Edge and evaluated with axe-core "
            f"({sAxeVersion}). Each axe rule is mapped to one or more "
            f"WCAG success criteria; the per-criterion rollup is "
            f"computed from rule outcomes (pass / fail / incomplete / "
            f"inapplicable) using the layered Calc precedence "
            f"described in the Methodology section."
        )
        d.add_paragraph(
            "Manual testing: criteria not fully decidable by automated "
            "rules (a substantial fraction of WCAG, including most "
            "criteria related to meaning, alternatives, and time-based "
            "media) require human review. ACR.xlsx provides per-criterion "
            "manual-test guidance for every WCAG 2.2 success criterion. "
            "The reviewer is expected to complete those checks and "
            "update the corresponding entries in ACR.xlsx before "
            "treating the ACR as final."
        )

        # ---- Section 2c: Terms ----
        # Mirrors the "Terms" section in the Zoom VPAT.
        d.add_heading("Terms", level=2)
        d.add_paragraph(
            "The conformance levels used in this report are defined as follows:"
        )
        for sTerm, sDef in [
            ("Supports",
             "The functionality of the page meets the criterion without "
             "known defects, or meets it with equivalent facilitation. "
             "Includes vacuous satisfaction when no content of the kind "
             "the criterion governs is present on the page."),
            ("Partially Supports",
             "Some functionality of the page meets the criterion; some does not."),
            ("Does Not Support",
             "The majority of the page's relevant functionality does not "
             "meet the criterion."),
            ("Not Applicable",
             "The criterion is not relevant to the page (for example, no "
             "rule produced any result for the criterion and the criterion "
             "concerns content that does not exist on the page)."),
            ("Not Evaluated",
             "The criterion has not yet been evaluated. Most commonly used "
             "for criteria that require manual review, or for criteria "
             "where automated rules produced an Incomplete outcome that "
             "could not be resolved without human judgement."),
        ]:
            p = d.add_paragraph(style="List Bullet")
        ...
        # ---- Section 8: About this report ----
        # Professional VPATs typically end with a "Notes" or "Legal
        # Disclaimer" or "About" section that establishes the
        # document's authorship and scope. We do something similar
        # here to make clear that this is automated output, not a
        # signed accessibility audit.
        d.add_heading("About this report", level=1)
        d.add_paragraph(
            f"This Accessibility Conformance Report was generated by "
            f"{sProgramName} {sProgramVersion}, an open-source Windows "
            f"command-line tool that drives Microsoft Edge through "
            f"Playwright and runs the axe-core rules engine on each "
            f"target page. The report is automated output and does not "
            f"by itself constitute an accessibility audit by an "
            f"accredited evaluator."
        )
        d.add_paragraph(
            f"Project home: https://github.com/JamalMazrui/urlCheck"
        )
        d.add_paragraph(
            f"{sProgramName} is released under the MIT License; the "
            f"reports it produces are owned by the user who ran the "
            f"scan. The {sProgramName} authors make no representation "
            f"about the conformance of any specific page or property; "
            f"the conformance verdicts in this document reflect "
            f"automated rule outcomes and are subject to verification "
